...
class Processor {
      ...
      return this.docClient.batchWrite(params)
        .then(unprocessedItems => {
          ...
          if (unprocessedItems) {
            ...
            if (items) return items.map(item => item.PutRequest.Item);
          }
        });
    };
  ...
  batchDelete(table, keys) {
    ...
    params.RequestItems[table] = keys.map((key) => {
        return {
            DeleteRequest: {
              Key: key
            }
    ...
    const f = () => {
      return this.docClient.batchWrite(params)
        .then(unprocessedItems => {
          this.logger.info('Batch deleted from %s table', table);

          if (unprocessedItems) {
            const items = unprocessedItems[table];
            if (items) return items.map(item => item.DeleteRequest.Key);
          }
        });
    };
